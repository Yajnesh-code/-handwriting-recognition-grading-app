...
# --- CONFIGURATION ---
DIGITS_MODEL_PATH = 'digits_model_experiment_1.keras'
LETTERS_MODEL_PATH = 'emnist_a_to_d_robust_classifier.keras'

DEBUG_SAVE_DIR = "debug_crops"
STATIC_DIR = "static"
ANSWER_KEY_DIR = "answer_keys"   # <--- NEW Folder for answer keys
...
LETTER_CLASS_NAMES = ['A', 'B', 'C', 'D']   # change to ['A','B','C','D','E'] if needed
...
def preprocess_char_for_model(char_gray):
    ...
    h, w = img_bin.shape
    if h > w: new_h, new_w = 20, max(1, int(round(w * 20.0 / h)))
    else: new_w, new_h = 20, max(1, int(round(h * 20.0 / w)))
...
def segment_digits(crop_gray):
    ...
    boxes = sorted([cv2.boundingRect(c) for c in contours if cv2.boundingRect(c)[3] > 6 and cv2.boundingRect(c)[2] > 2], key=lambda b: b[0])
...
def two_cluster_x(centers_x, iters=8):
    ...
    if len(xs) < 2: return np.ones(len(xs), dtype=bool)
    c1, c2 = xs.min(), xs.max()
    for _ in range(iters):
        assign = np.abs(xs - c1) < np.abs(xs - c2)
        if assign.sum() > 0: c1 = xs[assign].mean()
        if (~assign).sum() > 0: c2 = xs[~assign].mean()
    mean1 = xs[assign].mean() if assign.sum() > 0 else c1
    return assign if mean1 < (xs[~assign].mean() if (~assign).sum() > 0 else c2) else ~assign
...
def process_mcq_image(PAGE_IMAGE_PATH, exam_code):
    ...
    for c in contours:
        ...
        if w >= min_w and h >= min_h:
            candidates.append({'bbox': (x, y, w, h), 'cx': x + w / 2.0, 'cy': y + h / 2.0})

    if not candidates: return {"error": "No character candidates found."}

    centers_x = [c['cx'] for c in candidates]
    mask = two_cluster_x(centers_x)
    left_candidates = sorted([c for i, c in enumerate(candidates) if mask[i]], key=lambda c: c['cy'])
    right_candidates = sorted([c for i, c in enumerate(candidates) if not mask[i]], key=lambda c: c['cy'])
    ...
    for left, right in pairs:
        ...
        left_crop = gray[max(0, ly - pad):min(H, ly + lh + pad), max(0, lx - pad):min(W, lx + lw + pad)]

        digit_str = "".join([
            DIGIT_CLASS_NAMES[np.argmax(digits_model.predict(preprocess_char_for_model(ch)[0], verbose=0)[0])]
        ...
        if right:
            ...
            right_crop = gray[max(0, ry - pad):min(H, ry + rh + pad), max(0, rx - pad):min(W, rx + rw + pad)]
            if right_crop.size > 0:
                prepared, _ = preprocess_char_for_model(right_crop)
                predicted_letter = LETTER_CLASS_NAMES[np.argmax(letters_model.predict(prepared, verbose=0)[0])]
        ...
        cv2.putText(image_vis, label_text, (lx, ly - 12),
                 cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

        cv2.rectangle(image_vis, (lx, ly), (lx + lw, ly + lh), color, 2)
        if right:
           rx, ry, rw, rh = right['bbox']
           cv2.rectangle(image_vis, (rx, ry), (rx + rw, ry + rh), color, 2)
    ...
    filtered = [r for r in report_rows if r["question_pred"] in answer_key]
    score = sum(1 for r in filtered if r["result"] == "Correct")
    percentage = round((score / total_questions) * 100, 2) if total_questions > 0 else 0

    annotated_filename = "annotated_result.png"
